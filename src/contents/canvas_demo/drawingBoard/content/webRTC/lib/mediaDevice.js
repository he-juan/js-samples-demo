...
(function (root, factory) {
            ...
            if (sameWidthList.length > 0) {
                ...
                if (JSON.stringify(matchRes) === '{}') {
                    for (var k = 0; k < sameWidthList.length; k++) {
                        ...
                        if (sameWidthList[k].width === expectRes.width && sameWidthList[k].height === expectRes.height && sameWidthList[k].frameRate < expectRes.frameRate) {
                            log.log('Returns the resolution where the width height is the same and the frameRate is less than the expected value. ', sameWidthList[k])
                            matchRes = sameWidthList[k]
                            break
                        }
                    }
    ...
    /**
       * 判断共享桌面时是否支持共享音频
       * */
    MediaDevice.prototype.isSystemAudioShareSupport = function () {
        let result = false
    ...
    MediaDevice.prototype.getMedia = async function (data, constraints) {
        ...
        function onGetStreamFailed(error) {
            ...
            log.warn('onGetStreamFailed error message: ' + error.message)
            log.warn('error name: ' + error.name)
            log.warn('error constraint: ' + error.constraint)
            if (error.name === 'OverconstrainedError' || error.name === 'ConstraintNotSatisfiedError' || error.name === 'Error') {
                ...
                log.info('constraints can not be satisfied by avb.device')
                ...
                if (error.name === 'NotFoundError' || error.name === 'DeviceNotFoundError') {
                    ...
                    log.info('require track is missing')
                } else if (error.name === 'NotReadableError' || error.name === 'TrackStartError') {
                    // webcam or mic are already in use
                    log.info('webcam or mic are already in use')
                } else if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError' || error.name === 'PermissionDismissedError') {
                    // permission denied in browser
                    log.info('permission denied in browser')
                } else if (error.name === 'TypeError') {
                    // empty constraints object
                    log.info('empty constraints object')
                } else {
                    // other errors
                    log.info('other errors ' + error.name)
                }
    ...
    MediaDevice.prototype.getUserMediaWithTimeout = function (constrain, timeout = 10 * 1000) {
        log.info("current handle getUserMediaWithTimeout")
        return new Promise((res, rej) => {
            let isTimeOut = false;
            let timeOutTimer = null;
            timeOutTimer = setTimeout(() => {
                isTimeOut = true;
                let error = new Error('getUserMedia timeout');
                error.name = 'TimeOutError';
                error.constraint = constrain
                error.isAuth = window.Notification && Notification.permission
                log.warn("getUserMediaWithTimeout: timeout and isAuth: " + error.isAuth)
                rej(error);
            }, timeout);
            navigator.mediaDevices.getUserMedia(constrain)
                .then(stream => {
                    log.info("getUserMediaWithTimeout get stream success ")
                    if (isTimeOut) {
                        log.info("get stream:" + JSON.stringify(stream, null, '  '))
                ...
                .catch(error => {
                    error.isAuth = window.Notification && Notification.permission
                    error.constraint = constrain
                    log.info("getUserMediaWithTimeout get stream failed, isAuth: " + error.isAuth)
                    if (!isTimeOut) {
                        rej(error);
                    }
                })
    ...
    /***
       ...
       * 不支持IE 及 插件
       * @param data
       *  data.source: "window", "screen", "tab", "all"
    ...
    MediaDevice.prototype.getScreenShareConstraints = function (data) {
        ...
        if (navigator.mediaDevices.getDisplayMedia) {
            screenConstraints = {
                audio: this.isSystemAudioShareSupport(),
                video: {
                    width: { max: '1920' },
                    height: { max: '1080' },
                    frameRate: { max: '15' }
        ...
        //  */
        // if ((MediaDevice.getBrowserDetail().browser === "ie" || MediaDevice.getBrowserDetail().browser === "safari")) {
        //     screenConstraints = {
        //         audio: false,
        //         video: {
