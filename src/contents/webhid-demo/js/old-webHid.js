...
WebHID.prototype.resetState = function(){
    ...
    this.sendDeviceReport({ command: 'onHook', reset: true })
    this.sendDeviceReport({ command: 'muteOff', reset: true })
}
...
WebHID.prototype.sendDeviceReport = function (data){
    ...
    switch (data.command){
        case 'muteOn':
        case 'muteOff':
            This.device.muted = data.command === 'muteOn'
            break
        case 'incomingCall':
        case 'onHook':
        case 'offHook':
            if(!data.reset){
                // TODO: When the device state is restored, the oninputreport event of onHook will be triggered, causing the call to hang up
                setTimeout(function(){
                    if(data.command === 'onHook'){
                        This.device.hookStatus = 'on'
                        log.info('set onHook status')
                    }else if(data.command === 'offHook'){
                        This.device.hookStatus = 'off'
                        log.info('set offHook status')
                    }else if(data.command === 'incomingCall'){
                        This.device.hookStatus = 'incomingCall'
                        log.info('set incoming call state')
                    }
                }, 1000)
            }else {
                log.info("restore device state")
                This.device.hookStatus = null
            }
            break
        default:
            break
        ...
        if(reportId === 0x01){
            switch (reportData) {
                case inputReport.onHook.reportData:
                    if (This.device.hookStatus === 'off' || This.device.hookStatus === 'incomingCall') {
                ...
                case inputReport.offHook.reportData:
                    if (This.device.hookStatus === 'on' || This.device.hookStatus === 'incomingCall') {
