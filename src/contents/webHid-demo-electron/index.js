...
async function fistLine(){
    // await webHid.resetState()
    await webHid.sendDeviceReport({ command: 'offRing' })
    await webHid.sendDeviceReport({ command: 'offHook' })
    ledIncomingCall.checked = false
    ledOffHook.checked = true;
}
function unhold(){
	...
	switch (data.eventName){
		case 'ondevicehookswitch':
			if(data.hookStatus === 'on'){
                ...
                setTimeout(function(){  // Solve the problem of multi-line offhook triggering multiple events
                    console.warn("回复之前的状态")
                    webHid.sendDeviceReport({ command: 'offHook' })
                    ledOffHook.checked = true;
                },500)
			}else {
